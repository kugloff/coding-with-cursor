...
9. **Assistant tool JSON** — if changing **`edit_file`** / **`create_file`**, update **`server/assistantOutput.js`**, **`server/services/geminiService.js`** (prompt + **`RESPONSE_FORMAT_RULES`**), **`ChatPanel`**, **`App.jsx`** (`applyAiFileEdit`, **`AiEditPreviewModal`**, toast, **`editorNonce`**), **`CodeEditor`**, and this README.
10. **`POST /run`** — if changing execution: **`server/runCode.js`** (**vm2**, timeouts, **`MAX_RUN_CODE_CHARS`**), **`server/index.js`** route, Vite proxy **`/run`**, **`App.jsx`** **Run** / **Output** panel (minimize chevron, **`run-output--minimized`** in **`App.css`**), and this README.
...
| `src/App.jsx` | Workspace + **topbar** (**Undo** / **Redo**); **`applyAiFileEdit`** (**`edit_file`** / **`create_file`**) after diff **Accept**; **`AiEditPreviewModal`**; **`editorNonce`**, **`.ai-toast`** (full line text); **`POST /run`** for JS (**Run** + **Output** under Monaco, **minimize/expand** output via header chevron) |
...
| `toolCall` | `null` \| object | `null` for normal chat. Otherwise `{ "action": "edit_file" \| "create_file", "filename": string, "content": string }` after server-side validation of the parsed JSON. |
...
- **Success (200):** **`{ "output": string, "error": string }`**. User code runs inside **`vm2`** `VM` (**`executeJavaScript`** in **`server/runCode.js`**) with a stub **`console`** (`log`, `info`, `warn`, `error`, `debug`, `trace`) whose lines are joined into **`output`**. Wall-clock limit **`RUN_TIMEOUT_MS`** (**10 000** ms). Uncaught exceptions or timeouts set **`error`** to the message; partial **`console`** lines may still appear in **`output`**. When the script’s completion value is not `undefined`/`null`, a stringified tail is appended to **`output`** (after console lines).

**Security note:** **`vm2` is unmaintained** and does not provide strong guarantees against a determined attacker. Treat **`POST /run`** as appropriate for **local demos** and **low-trust** environments only; production or hostile input should use stronger isolation (separate process, container, resource limits, review).
...
│   ├── assistantOutput.js # parseAssistantModelOutput(raw) — text vs edit_file / create_file JSON
│   ├── chatBody.js       # parseChatContext(files, currentFile) for POST /chat
│   ├── runCode.js        # executeJavaScript(code) — vm2 VM, console capture, timeout
...
- **Run JS:** `POST /run` → **`executeJavaScript`** in **`server/runCode.js`** (**vm2**); **`App.jsx`** **Run** + **Output** panel (header chevron **minimizes** / **expands** the output body; **Run** re-expands when you execute).
- **Chat + code context:** `POST /chat` with **`files`** / **`currentFile`**; **`geminiService.js`** builds list + active-first prompt; **`assistantOutput.js`** parses **`edit_file`** / **`create_file`**; **`App.jsx`** + **`AiEditPreviewModal`** (accept/reject diff) then **`applyAiFileEdit`**, **`editorNonce`**, **`.ai-toast`**; **`ChatPanel`** uses **`onAiEditProposal`**.
...
| Run button disabled | Active file is not JavaScript (open `.js` / `.jsx` / `.mjs` / `.cjs`) |
